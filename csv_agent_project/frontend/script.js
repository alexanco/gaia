...
function setupEventListeners() {
    ...
    setInterval(checkApiStatus, 60000); // A cada minuto
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.status === 'healthy') {
                ...
                updateApiStatus('offline', 'API Indisponível');
            }
        } else {
            updateApiStatus('offline', 'API Indisponível');
        }
        ...
        if (error.name === 'AbortError') {
            console.warn('Verificação de status da API cancelada por timeout');
        } else {
...
function updateApiStatus(status, text) {
    ...
    // Habilitar/desabilitar input baseado no status
    const isOnline = status === 'online';
    elements.messageInput.disabled = !isOnline;
    elements.sendButton.disabled = !isOnline || appState.isLoading;
    
    if (!isOnline) {
        elements.messageInput.placeholder = 'API indisponível no momento...';
    } else {
        elements.messageInput.placeholder = 'Digite sua pergunta sobre notas fiscais...';
    }
}
...
function handleInputChange(e) {
    ...
    const canSend = length > 0 && length <= 500 && appState.apiStatus === 'online' && !appState.isLoading;
...
async function sendMessage() {
    ...
    if (!message || appState.isLoading || appState.apiStatus !== 'online') {
        return;
    }
...
function sendSuggestion(suggestion) {
    if (appState.isLoading || appState.apiStatus !== 'online') {
        return;
    }
...
function setLoading(loading) {
    ...
    elements.messageInput.disabled = appState.apiStatus !== 'online' || loading;
    elements.sendButton.disabled = appState.apiStatus !== 'online' || loading || elements.messageInput.value.trim().length === 0;
}
...
function handleApiError(error) {
    ...
    // Verificar status da API novamente
...
// Focar no input
function focusInput() {
    if (appState.apiStatus === 'online' && !appState.isLoading) {
